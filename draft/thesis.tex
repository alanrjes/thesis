...
\section{A closer look at system-scope granularity}
	...
	\begin{figure}[h]
		\centering
		\begin{tikzpicture}[node distance = 1.5cm, every node/.style = {rectangle, rounded corners, draw=black, minimum width=4cm, minimum height=0.8cm, text centered}]
			\node (WriteAllocator) {\verb`WriteAllocator`};
			\node (BaseCache) [below of=WriteAllocator] {\verb`BaseCache`};
			\node (Cache) [below of=BaseCache] {\verb`Cache`};
			\node (System) [below of=Cache] {\verb`System`};
			\node[draw=none] (upperRight) [right of=BaseCache, xshift=1.4cm] {};
			\node[draw=none] (lowerRight) [right of=Cache, xshift=1.4cm, yshift=0.68cm] {};
			\node[draw=none] (upperLeft) [above left of=System, xshift=-1.8cm] {};
			\node[draw=none] (lowerLeft) [below left of=Cache, xshift=-1.8cm, yshift=-0.2cm] {};
			\draw[<-, dashed] (WriteAllocator) -- (BaseCache);
			\draw[<-] (BaseCache) -- (Cache);
			\draw[<-, dashed] (Cache) -- (System);
			\draw (WriteAllocator) -| (lowerRight);
			\draw[->] (upperRight) |- (System);
			\draw (Cache) -| (lowerLeft);
			\draw[->] (upperLeft) |- (System);
		\end{tikzpicture}
		\caption{Relationships of inheritance (Child {\textrightarrow} Parent) and definition (Caller $\dasharrow$ Instance) between certain Gem5 objects.}
	\end{figure}

	This indicates that lower-level that modifications made to a child class (eg. \verb`Write`-\verb`Allocator`) could impact the abstracted caller (eg. \verb`System`). Thus, it is unlikely that the line size of an individual cache could be modified without identifying and accounting for shared reference to line size at both higher and lower levels.
	...
	Thus, I will instead present a study of alternative options for cache simulation with a focus on their capacity for mixed granularity cache implementation.
