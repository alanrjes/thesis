...
self.cacheMemory = RubyCache(
	...
	The helper \verb`getBlockSizeBits` here is a simple conversion of the system-level line size value. Although its invocation here can't be manipulated to modify the line size, since it only determines the start bit offset, its presence suggests that the system line size may only be communicated to the cache level over a few select definitions. This indicates as a possible course of action the possibility of modifying these definitions within the existing cache classes to isolate and reassign their line size value.

	\subsection*{Limitations and implications}

	Since the Ruby caches, like the classic caches, operate under the assumption of a constant system-wide line size, there is no straightforward way to modify the granularity of individual memory layers using Ruby.
	...
	However, the definition of cache memory shown in Ruby suggests sufficient modularity as to potentially alter granularity at the level of cache class definitions. This approach could potentially be more easily applied to the classic cache subsystem.


\section{An attempt to override system granularity}

	My final approach to implementing IBLP using existing Gem5 caches is to modify the existing classic cache definitions to allow the line size of lower memory levels to be changed. In addition to allowing my initial classic cache implementation of IBLP to model granularity-change, this would also open the option of flexible granularity to other cache configurations using simple caches.
